...
for col in ['properties.REVENUE', 'properties.DISTANCE', 'properties.CUSTOMERS', 'properties.DONUTS_SOLD', 'properties.RATING']:
...
def create_map(selected_walk_time):
    ...
    if selected_walk_time == '5':
        ...
        for mins, poly, fillcolor, linecolor, legend in [
            ...
            lons, lats = zip(*polygon_coords)
            fig.add_trace(go.Scattermap(lon=lons, lat=lats, mode='lines', fill='toself', fillcolor=fillcolor, line=dict(width=2, color=linecolor), name=legend))
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([html.H3("Total Customers", style={"fontSize": "24px", "fontWeight": "bold"}), html.P(id="scorecard-customers", style={"fontSize": "42px", "fontWeight": "bold"})], style={"padding": "10px", "margin": "5px", "background": "#f9f9f9
